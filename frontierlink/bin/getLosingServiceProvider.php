...
function getLosingServiceProvider() {
  ...
  try{
  				$response = $client->getLosingServiceProvider($params);
     }
  ...
  while ($cel = each($response->losingServiceProviderList->losingServiceProvider)) {

	$arr[] = $cel['value']->serviceProviderName;
