...
///////////////////////////////////////////////////////////////////////////////
//
// htdocs/base/manage/billing/once_off/index.php - View Billing: Invoiced
// $Id$
//
///////////////////////////////////////////////////////////////////////////////
//
...
if ($user->class == 'customer') {
  ...
  $pt->setFile(array("outside" => "base/outside3.html", "main" => "base/manage/billing/once_off/invoiced/index.html"));
  
} else if ($user->class == 'admin') {
  
  $pt->setFile(array("outside" => "base/outside1.html", "main" => "base/manage/billing/once_off/invoiced/index.html"));
...
if ( $user->class == "reseller" ) {
  $key = "wholesale_invoice_id";
} else if ( $user->class == "admin" ) {
  $key = "main_invoice_id";
}

$total_entries = $service_billing_once_off->get_num_rows_invoice($user->class,$user->access_id,$key);
...
$once_off_arr = $service_billing_once_off->get_invoice($user->class,$user->access_id,$key,$start,$end);
...
for ($a=0; $a < count($once_off_arr); $a++) { 
  ...
  if ( $user->class == "reseller" && $user->access_id == $wholesaler->wholesaler_id && $once_off_arr[$a]["wholesale_invoice_id"] != 0 ) {
  ...
  } else if ( $user->class == "admin" && $once_off_arr[$a]["main_invoice_id"] != 0 ) {
    $pt->setVar("ITEM_ID",$once_off_arr[$a]["item_id"]);
    $pt->setVar("WHOLESALER",$wholesaler->company_name);
    $pt->setVar("WHOLESALER_ID",$wholesaler->wholesaler_id);
    $pt->setVar("CUSTOMER",$customer->first_name . " " . $customer->last_name);
...
$pt->setVar("PAGE_TITLE", "View Once-Off Billing Invoiced");
