...
///////////////////////////////////////////////////////////////////////////////
//
// htdocs/base/manage/services/edit/confirm/index.php - Create a service
// $Id$
//
///////////////////////////////////////////////////////////////////////////////
//
...
if ($user->class == 'customer') {
  ...
  $pt->setFile(array("outside" => "base/outside2.html", "main" => "base/manage/services/edit/confirm/index.html"));
  
} else if ($user->class == 'reseller') {
  $pt->setFile(array("outside" => "base/outside3.html", "main" => "base/manage/services/edit/confirm/index.html"));
  
} else if ($user->class == 'admin') {
  $pt->setFile(array("outside" => "base/outside1.html", "main" => "base/manage/services/edit/confirm/index.html"));
...
if ( isset($_REQUEST['submit']) ) {
  ...
  if ( count($error_order) > 0 ) {
    ...
    if ( $create_for_addon == 1 ) {
      ...
      if ( isset($_REQUEST["order_address"]) ) {
        $order_attr->value = $_REQUEST["order_address"];
      }
      $order_attr->create();
    ...
    // Send order receipt:
      ...
      $text_body .= "New Plan: " . $new_plan->description . "\r\n";
      $text_body .= "Contract Term (Months): " . $contract_length->value . "\r\n\r\n";

      $text_body .= "Service Number: " . $services->identifier . "\r\n";
      if ( $services->type_id == '1' || $services->type_id == '2' ) {
        $text_body .= "Access Location: " . get_service_attribute($services->service_id,"address") . "\r\n";
        $text_body .= "Access Method: " . $new_plan->access_method . "\r\n";
        ...
        $text_body .= "Access Speed: Up to " . $new_plan->speed . "\r\n\r\n";
      }
      ...
      if ( $create_for_addon == 1 ) {
        ...
        $comment->username = $user->username;
        $comment->comment_visibility = "customer";
        $comment->comment = "Addon Order: http://simplicity.xi.com.au/base/manage/orders/edit/?order_id=".$addon_order->order_id;
        $comment->create();

        $comment = new order_comments();
        $comment->order_id = $addon_order->order_id;
        $comment->username = $user->username;
        $comment->comment_visibility = "customer";
        $comment->comment = "Main Order: http://simplicity.xi.com.au/base/manage/orders/edit/?order_id=".$orders->order_id;
        $comment->create();
      }
...
$pt->setVar("NEW_PLAN",$new_plan->description);
