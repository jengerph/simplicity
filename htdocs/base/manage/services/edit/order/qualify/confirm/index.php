...
///////////////////////////////////////////////////////////////////////////////
//
// htdocs/base/manage/services/edit/order/qualify/confirm/index.php - Create a service
// $Id$
//
///////////////////////////////////////////////////////////////////////////////
//
...
if ($user->class == 'customer') {
  ...
  $pt->setFile(array("outside" => "base/outside2.html", "main" => "base/manage/services/edit/order/qualify/confirm/index.html"));
  ...
  $pt->setFile(array("outside" => "base/outside3.html", "main" => "base/manage/services/edit/order/qualify/confirm/index.html"));
  
} else if ($user->class == 'admin') {
  $pt->setFile(array("outside" => "base/outside1.html", "main" => "base/manage/services/edit/order/qualify/confirm/index.html"));
  
}

$pt->setFile(array("wholesaler_row" => "base/manage/services/edit/order/qualify/confirm/wholesaler_row.html","retailer_row" => "base/manage/services/edit/order/qualify/confirm/retailer_row.html","churn" => "base/manage/services/edit/order/qualify/confirm/churn.html", "add_contact" => "base/manage/services/add/qualify/creation/add_contact.html"));

$customer = new customers();
$customer->customer_id = $services->customer_id;
$customer->load();

$osa = str_replace("Up to ", "", trim($_SESSION['order_service_available']));
$temp = explode(" ", trim($_SESSION['order_service_available']));

$st = new service_types();
$st->description = $temp[0];
$st->get_type_id();
...
if ( isset($_REQUEST["order_churn_provider"]) ) {
  $pt->setVar('ORDER_CHURN_' . strtoupper($_REQUEST["order_churn_provider"]), ' selected');
}
...
function clean_array($array,$temp){
  ...
  $param = array("subAddressType","subAddressNumber","streetNumber","streetName","streetType","suburb","state","postcode","qualificationID","nbnLocationID","dslCodesOnLine","type","distanceToExchange","accessMethod","accessType","priceZone","serviceSpeed");
  $result = array();
  ...
  $array2 = array_keys($array["results"][$temp[0]]);

  //find serviceSpeed
  for ($a=0; $a < count($array2); $a++) { 
    if ( is_array($array["results"][$temp[0]][$array2[$a]]) ) {
      if ( is_object($array["results"][$temp[0]][$array2[$a]]) ) {
        $value = (array)$array["results"][$temp[0]][$array2[$a]];
      } else{
        $value = $array["results"][$temp[0]][$array2[$a]];
      }
      if ( is_array($array["results"][$temp[0]][$array2[$a]]["availableServiceSpeeds"]->serviceSpeed) ) {
        for ($b=0; $b < count($array["results"][$temp[0]][$array2[$a]]["availableServiceSpeeds"]->serviceSpeed); $b++) { 
          ...
          if ( preg_match("#$var#",$array["results"][$temp[0]][$array2[$a]]["availableServiceSpeeds"]->serviceSpeed[$b]->serviceSpeed) && ($array2[$a] == $temp[1]) ) {
            $result["serviceSpeed"] = $array["results"][$temp[0]][$array2[$a]]["availableServiceSpeeds"]->serviceSpeed[$b]->serviceSpeed;
            $parent = $array2[$a];
          }
        ...
        if ( preg_match("#$var#",$array["results"][$temp[0]][$array2[$a]]["availableServiceSpeeds"]->serviceSpeed->serviceSpeed)) {
            $result["serviceSpeed"] = str_replace("Up to ", "", $array["results"][$temp[0]][$array2[$a]]["availableServiceSpeeds"]->serviceSpeed->serviceSpeed);
            $parent = $array2[$a];
          }
      }
      // exit();
...
$result["maximumDownBandwidth"] = $array["results"][$temp[0]][$parent]["maximumDownBandwidth"]->value  . " " . $array["results"][$temp[0]][0]["maximumDownBandwidth"]->quantifier;
$result["maximumUpBandwidth"] = $array["results"][$temp[0]][$parent]["maximumUpBandwidth"]->value  . " " . $array["results"][$temp[0]][0]["maximumUpBandwidth"]->quantifier;
$result["type"] = $array["results"][$temp[0]][$parent]["type"];
$result["distanceToExchange"] = $array["results"][$temp[0]][$parent]["distanceToExchange"];
$result["accessMethod"] = $array["results"][$temp[0]][$parent]["accessMethod"];
$result["accessType"] = $array["results"][$temp[0]][$parent]["accessType"];
$result["priceZone"] = $array["results"][$temp[0]][$parent]["priceZone"];
