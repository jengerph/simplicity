...
$params['qualifyNationalWholesaleBroadbandProductRequest']['endCSN'] = '0263696233';
...
/* Values are:
            ...
            <xsd:enumeration value="NoisyLine" />
            <xsd:enumeration value="LowNoisyLine" />
            <xsd:enumeration value="ExtremelyNoisyLine" />
            */
function dsl_changeProfile ($serviceOrderID,$profile) {

$arr = changeProfile($serviceOrderID,$profile);
...
function changeProfile($serviceid, $profile) {
  ...
  $params['serviceOrderID'] = $serviceid;
  $params['lineProfile'] = $profile;
  
  
  try{
  				$response = $client->changeProfile($params);
         //$response = $client->__soapCall("sendMessages", array($params));           
     }
  catch (SoapFault $exception) {
